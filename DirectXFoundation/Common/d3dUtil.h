...
private:
	...
	Microsoft::WRL::ComPtr<ID3D12Resource> GPUBuffer = nullptr;
	Microsoft::WRL::ComPtr<ID3D12Resource> UploaderBuffer = nullptr;

	UINT ByteStride = 0;
	UINT BufferByteSize = 0;
};

class StaticMeshIndexBuffer
{
public:
	StaticMeshIndexBuffer() = default;
	StaticMeshIndexBuffer(const void* indexData, const UINT bufferByteSize, const DXGI_FORMAT indexFormat = DXGI_FORMAT_R16_UINT);
...
struct StaticMeshGeometry
{
	// Give it a name so we can look it up by name.
	std::string Name;

	std::unordered_map<std::string, StaticMeshVertexBuffer> VertexBuffers;
